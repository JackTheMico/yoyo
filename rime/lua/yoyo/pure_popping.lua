...
--    e. 四码非词自动回退 (如 slcb)：若 slcb 不是词库中的 4 码词 -> 顶出 sl ("了")，留 cb ("不")
...
function processor.func(key_event, env)
  ...
  -- 辅助函数：提交前部，保留后部
  local function commit_and_retain(first_part, remainder)
    env.processing = true
    context:pop_input(#input)
    context:push_input(first_part)
    if context:has_menu() then
      context:confirm_current_selection()
      context:commit()
      context:clear()
    else
      env.engine:commit_text(first_part)
      context:clear()
    end
  ...
  -- Pattern A: 1简接1简 (如 _d_e, +e_e, _d+e, +e+e)
  local jian1, jian2 = input:match("^([_+][^_%s])([_+][^_%s])$")
  if jian1 and jian2 then
    return commit_and_retain(jian1, jian2)
  end
  ...
  if chord2 and jian then
    ...
    if pure_words.chars_3code[candidate_3code] then
      -- 3码单字直出模式
      if context:get_option("instant_commit_3code") and context:has_menu() then
        return commit_and_retain(input, "")
      end
      return yoyo.kNoop
    else
      -- 否则是两码字接1简 (如 iG+e "打"+"有") -> 立即提交两码字，留1简！
      return commit_and_retain(chord2, jian)
    end
  end
  ...
  if char3 and next_stroke then
    return commit_and_retain(char3, next_stroke)
  end

  -- Pattern D: 4码输入 (如 rTah, ISoY, slcb)
  if #input == 4 and not input:find("[_+]") then
    -- 如果是词库中的 4 码词 (如 rTah "类似", ISoY "空格", xkhr "可以")
    if pure_words.words_4code[input] then
      return yoyo.kNoop  -- 完整词语，保持不拆！
    else
      -- 4码非词回退 (如 slcb "了不") -> 提交前2码 (sl "了")，留后2码 (cb "不")
      local first_two = input:sub(1, 2)
      local last_two = input:sub(3, 4)
      return commit_and_retain(first_two, last_two)
    end
  end
  ...
  if word4 and next_stroke4 and not word4:find("[_+]") then
    return commit_and_retain(word4, next_stroke4)
  end
